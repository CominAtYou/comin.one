...
async function getDefaultAvatar(env: Env) {
    ...
    if (cachedAvatarHash !== null) {
        return Response.redirect(`https://cdn.discordapp.com/avatars/${await env.CREDS.get("DISCORD_ID")}/${cachedAvatarHash}?size=1024`, 302);
    }
    ...
    return Response.redirect(`https://cdn.discordapp.com/avatars/${await env.CREDS.get("DISCORD_ID")}/${response.avatar}?size=1024`, 302);
}
...
async function getGuildAvatar(id: string, env: Env): Promise<Response> {
    ...
    if (cachedGuildAvatarHash === "default") {
        ...
        return Response.redirect(`https://discord.com/guilds/${id}/users/${await env.CREDS.get("DISCORD_ID")}/${cachedGuildAvatarHash}?size=1024`, 302);
    }
    ...
    if (response.avatar === null) {
        ...
        return Response.redirect(`https://cdn.discordapp.com/guilds/${id}/users/${await env.CREDS.get("DISCORD_ID")}/avatars/${response.avatar as string}?size=1024`);
    }
}